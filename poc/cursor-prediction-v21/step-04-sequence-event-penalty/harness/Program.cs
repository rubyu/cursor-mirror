...
static class Program
{
    ...
    static int Main(string[] args)
    {
        ...
        var trainRows = SampleTrainRows(rows.Where(r => r.Split == "train").ToArray(), config.TrainSampleCap, config.Seed);
        var candidates = new Dictionary<string, object>();
        var trainedModels = new Dictionary<string, TrainedMlp>();
        foreach (var spec in GenerateModelSpecs())
        {
        ...
        candidates["mlp_h32_event_safe_seq_latch_cap0p35"] = EvaluateGuardedModel(
            rows,
            new CandidateSpec(
                "mlp_h32_event_safe_seq_latch_cap0p35",
                "MLP+SequenceGuard",
                "temporal+runtime-state",
                32,
                "best step03 event-safe MLP with a post-model stop/static latch cap",
                "event-safe MLP output capped to 0.35 px while recentHigh/low-v/near-stop latch is active",
                "postModelGuard",
                StopCapPx: 0.35),
            trainedModels["mlp_h32_event_safe_sampled"]);

        candidates["mlp_h32_asym_seq_latch_blend0p50_cap0p45"] = EvaluateGuardedModel(
            rows,
            new CandidateSpec(
                "mlp_h32_asym_seq_latch_blend0p50_cap0p45",
                "MLP+SequenceGuard",
                "temporal+runtime-state",
                32,
                "asymmetric-lead MLP with stop-window lag/lead guard and cap",
                "asymmetric MLP blended 50% toward current and capped to 0.45 px while stop latch is active",
                "postModelGuard",
                StopCapPx: 0.45),
            trainedModels["mlp_h32_asymmetric_lead_sampled"],
            stopBlend: 0.5);
        ...
        candidates["rule_hybrid_latch_v5_300_high400_latest2p5"] = EvaluateRuleHybrid(
            ...
            "v20 rule: recentHigh>=400, v5<=300, latestDelta<=2.5; snap to current while latched",
            useV5ForStart: true,
            highMin: 400,
            v2Max: 300,
            latestMax: 2.5,
        ...
        candidates["rule_hybrid_cap0p5_v2_150_high400_latest2p0"] = EvaluateRuleHybrid(
            ...
            "v20 rule: recentHigh>=400, v2<=150, latestDelta<=2.0; cap prediction magnitude to 0.5 px while latched",
            capPx: 0.5,
            highMin: 400,
            v2Max: 150,
            latestMax: 2.0,
        ...
        var ranking = candidates.Select(kvp =>
        {
            var doc = JsonSerializer.SerializeToElement(kvp.Value, JsonOptions);
            double objective = doc.GetProperty("objective").GetProperty("total").GetDouble();
            return new { id = kvp.Key, objective };
        }).OrderBy(x => x.objective).ToArray();
        var productComparison = BuildProductComparison(candidates, "product_distilled_lag0_offset_minus4_brake");

        var output = new
        {
            schemaVersion = "cursor-prediction-v21-step-04-sequence-event-penalty/1",
            generatedAtUtc = DateTimeOffset.UtcNow,
            environment = new
            {
                runtime = ".NET " + Environment.Version,
                execution = "CPU-only serialized reevaluation/training",
            ...
            {
                seed = config.Seed,
                trainRowsBeforeCap = rows.Count(r => r.Split == "train"),
            ...
            conclusionHints = new
            {
                ...
                productRank = Array.FindIndex(ranking, x => x.id == "product_distilled_lag0_offset_minus4_brake") + 1,
                conservativeGate = "No candidate should be considered product-ready unless it improves balanced stop-event risk without degrading normal visual p95/p99 on the untouched normal test split."
            }
        };
        ...
        Console.WriteLine($"rows={rows.Count} trainUsed={trainRows.Length} candidates={candidates.Count} elapsedMs={sw.Elapsed.TotalMilliseconds:0}");
        return 0;
    }
    ...
    static Config DefaultConfig() => new(
        ...
        ScoresPath: "poc/cursor-prediction-v21/step-04-sequence-event-penalty/scores.json",
        WarmupMilliseconds: 1500,
        Seed: 2104,
        TrainSampleCap: 80000,
        MaxEpochs: 60,
        LearningRate: 0.003);
    ...
    static void MarkEventWindows(List<DataRow> rows)
    {
        ...
        foreach (var group in rows.GroupBy(r => r.ScenarioKey))
        {
            ...
            {
                bool stat = IsHold(ordered[i]) || (ordered[i].F.V12 <= 100 && ordered[i].F.TargetDistance <= 0.75);
    ...
    static DataRow[] SampleTrainRows(DataRow[] trainRows, int cap, int seed)
    {
        ...
        return trainRows
            .Select((row, i) => (row, key: StableRandom(seed, row.PackageId, row.ScenarioIndex, i)))
            .OrderBy(x => x.key)
            .Take(cap)
            .Select(x => x.row)
            .ToArray();
    ...
    static IEnumerable<CandidateSpec> GenerateModelSpecs()
    {
        yield return new CandidateSpec(
            ...
            "temporal",
            32,
            "event-safe target; post-stop and stationary labels pull to zero displacement",
            "one hidden tanh MLP, hidden=32, CPU compact search",
            "eventSafe");
        yield return new CandidateSpec(
            ...
            "temporal",
            32,
            "asymmetric future-lead penalty on manifest train split",
            "one hidden tanh MLP, hidden=32, lead penalty 8/event multiplier 2/lag 0.5",
            "asymmetricLead",
            LeadWeight: 8,
            EventLeadMultiplier: 2,
            LagWeight: 0.5,
            EventWeight: 1.5,
            StaticWeight: 2.0);
        yield return new CandidateSpec(
            "mlp_h32_event_peaklead_hinge_stoprows",
            "MLP",
            "temporal",
            32,
            "event-safe target plus lead-projection hinge on stop-window rows",
            "one hidden tanh MLP, hidden=32, peakLead hinge cap 0.25 px on stop windows",
            "eventPeakLeadHinge",
            EventWeight: 14,
            StaticWeight: 3,
            PeakLeadCap: 0.25,
            PeakLeadHingeWeight: 18,
            JitterWeight: 1.5);
        yield return new CandidateSpec(
            "mlp_h32_event_returnmotion_proxy",
            "MLP",
            "temporal",
            32,
            "event-safe target plus returnMotion proxy penalty on stop/static rows",
            "one hidden tanh MLP, hidden=32, stop-window magnitude proxy and static suppression",
            "eventReturnMotionProxy",
            EventWeight: 12,
            StaticWeight: 3,
            PeakLeadCap: 0.35,
            PeakLeadHingeWeight: 8,
            ReturnMotionWeight: 10,
            JitterWeight: 3);
        yield return new CandidateSpec(
            "mlp_h32_asym_lagguard_jitter",
            "MLP",
            "temporal",
            32,
            "asymmetric lead loss only in event windows, normal-motion lag guard, and stationary jitter penalty",
            "one hidden tanh MLP, hidden=32, event lead hinge with lag guard >=0.65 px on normal moving rows",
            "asymLagGuardJitter",
            LeadWeight: 10,
            EventLeadMultiplier: 3,
            LagWeight: 0.8,
            EventWeight: 2,
            StaticWeight: 3,
            PeakLeadCap: 0.3,
            PeakLeadHingeWeight: 12,
            LagGuardWeight: 10,
            LagGuardPx: 0.65,
            JitterWeight: 5);
        yield return new CandidateSpec(
            "mlp_h32_stop_safety_target_cap",
            "MLP",
            "temporal",
            32,
            "stop-window safety target with capped displacement when recentHigh is high and current motion is near stop",
            "one hidden tanh MLP, hidden=32, differentiable cap proxy against nonzero stop output",
            "stopSafetyTarget",
            EventWeight: 18,
            StaticWeight: 3,
            PeakLeadCap: 0.2,
            PeakLeadHingeWeight: 16,
            ReturnMotionWeight: 6,
            JitterWeight: 4,
            StopCapPx: 0.25);
    }
    ...
    static TrainedMlp TrainMlp(CandidateSpec spec, DataRow[] trainRows, Config config)
    {
        ...
        var model = new MlpModel(x[0].Length, spec.Hidden, new Random(config.Seed + spec.Id.Length));
        ...
        for (int epoch = 1; epoch <= config.MaxEpochs; epoch++)
        {
            ...
            {
                var row = trainRows[i];
                bool stopSafety = IsStopSafetyRow(row);
    ...
    static object EvaluateGuardedModel(List<DataRow> rows, CandidateSpec spec, TrainedMlp model, double stopBlend = 0.0)
    {
        var preds = new Dictionary<DataRow, double[]>();
        foreach (var group in rows.GroupBy(r => r.ScenarioKey))
        {
            ...
            foreach (var row in group.OrderBy(r => r.ScenarioElapsedMs))
            {
                ...
                bool start = row.EventWindowLabel || IsStopSafetyRow(row);
                bool release = row.F.V2 > 220 || row.F.LatestDelta > 3.0 || row.F.TargetDistance > 1.5;
                if (start) latch = 10;
                if (release) latch = 0;

                double dx = raw[0], dy = raw[1];
                if (row.StaticLabel && row.F.V12 <= 100 && row.F.TargetDistance <= 0.75)
                {
                    dx = 0;
                    dy = 0;
                }
        ...
        return EvaluateCandidate(
            ...
            new
            {
                ...
                guard = "per-scenario stop latch, static zero target, cap, and lead-side clamp",
                stopBlend,
                spec.StopCapPx
            });
    }
    ...
    static bool IsStopSafetyRow(DataRow r) =>
        r.F.RecentHigh >= 400 &&
        r.F.V2 <= 140 &&
        r.F.LatestDelta <= 2.5 &&
        r.F.TargetDistance <= 1.25;
    static bool IsNormalMoving(DataRow r) =>
        !r.EventWindowLabel &&
        !r.StaticLabel &&
        r.F.V12 >= 250 &&
        r.F.V12 < 1800 &&
        r.F.TargetDistance > 0.5;
    ...
    static EvalRow BuildEval(string id, DataRow r, double dx, double dy)
    {
        ...
        bool acute = (r.F.V12 >= 500 && r.F.TargetDistance <= 0.75) ||
            (r.F.V12 >= 800 && r.F.V2 <= r.F.V12 * 0.35) ||
            (r.F.V12 >= 1500 && r.F.TargetDistance <= 0.75) ||
            (r.F.V5 >= 500 && r.F.V2 <= 100) ||
            (r.F.V12 >= 500 && r.F.V5 <= 250 && r.F.V2 <= 100);
        bool high = r.F.V12 >= 1800;
        bool stat = IsHold(r) || (r.F.V12 <= 100 && r.F.TargetDistance <= 0.75);
        bool normal = !acute && r.F.V12 >= 250 && r.F.V12 < 1800;
        return new EvalRow(id, r.PackageId, r.Split, r.QualityBucket, r.DurationBucket, r.ScenarioIndex, r.ScenarioKey, r.ScenarioElapsedMs, r.MovementPhase, visual, visual * visual, Math.Max(0, signedFuture), Math.Max(0, -signedFuture), dist, dx, dy, r.F.DirX, r.F.DirY, r.F.V2, r.F.V5, r.F.V8, r.F.V12, r.F.RecentHigh, r.F.LatestDelta, r.F.TargetDistance, normal, high, stat);
    }
    ...
    static Features RuntimeFeatures(CallPoint call, List<CallPoint> history, (double X, double Y, double Vx, double Vy, double Speed) target)
    {
        ...
        return new Features(v2.Dx, v2.Dy, v2.Speed, v3.Dx, v3.Dy, v3.Speed, v5.Dx, v5.Dy, v5.Speed, v8.Dx, v8.Dy, v8.Speed, v12.Dx, v12.Dy, v12.Speed, recentHigh, latestDelta, dirX, dirY, path.Net, path.Path, path.Efficiency, horizonMs, Dist(call.X, call.Y, target.X, target.Y), target.Speed);
    }
