...
static class Program
{
    ...
    static int Main(string[] args)
    {
        ...
        var output = new
        {
            schemaVersion = "cursor-prediction-v21-step-03-balanced-reevaluation/1",
    ...
    static Config DefaultConfig() => new(
        ...
        ScoresPath: "poc/cursor-prediction-v21/step-03-balanced-reevaluation/scores.json",
    ...
    static TrainedMlp TrainMlp(CandidateSpec spec, DataRow[] trainRows, Config config)
    {
        ...
        for (int epoch = 1; epoch <= config.MaxEpochs; epoch++)
        {
            ...
            {
                var row = trainRows[i];
                bool safe = spec.LossKind == "eventSafe" && (row.EventWindowLabel || row.StaticLabel);
