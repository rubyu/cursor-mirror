...
record CandidateRun(
    ...
    bool FutureLagWithinEpsilon,
    bool PassesIntegrationGate,
...
static class Program
{
    ...
    static int Main(string[] args)
    {
        ...
        foreach (int seed in config.Seeds)
        {
            ...
            AddRun(candidateRuns, seed, productMetrics, config.FutureLagEpsilon, EvaluateCandidate(
                rows,
                eventSafeSpec,
            ...
            {
                AddRun(candidateRuns, seed, productMetrics, config.FutureLagEpsilon, EvaluateGuardedModel(
                    rows,
                    guarded.Spec,
                    model,
        ...
        var ranking = candidateRuns
            ...
            .Select(g => new
            {
                ...
                passCount = g.Count(r => r.PassesIntegrationGate)
            })
        ...
        var output = new
        {
            schemaVersion = "cursor-prediction-v21-step-05-multiseed-event-safe-validation/1",
            ...
            conclusionHints = new
            {
                ...
                productMetrics,
                gate = "Normal visual p95/p99 may match product; robustness peakLead/OTR/returnMotion and futureLead must beat product; futureLag p95 may not exceed product by epsilon."
            }
        };
    ...
    static Config DefaultConfig() => new(
        ...
        ScoresPath: "poc/cursor-prediction-v21/step-05-multiseed-event-safe-validation/scores.json",
    ...
    static DataRow[] SampleTrainRows(DataRow[] trainRows, int cap, int seed)
    {
        ...
        var events = trainRows.Where(r => r.EventWindowLabel || IsStopSafetyRow(r)).ToArray();
        var statics = trainRows.Where(r => r.StaticLabel && !(r.EventWindowLabel || IsStopSafetyRow(r))).ToArray();
    ...
    static object TrainStrataSummary(DataRow[] rows) => new
    {
        ...
        eventOrStopSafety = rows.Count(r => r.EventWindowLabel || IsStopSafetyRow(r)),
        staticRows = rows.Count(r => r.StaticLabel),
        other = rows.Count(r => !IsNormalMoving(r) && !(r.EventWindowLabel || IsStopSafetyRow(r)) && !r.StaticLabel)
    };

    static IEnumerable<CandidateSpec> GenerateModelSpecs()
    {
        yield return new CandidateSpec(
            "mlp_h32_event_safe_fulltrain",
            "MLP",
            "temporal",
            32,
            "event-safe target; post-stop and stationary labels pull to zero displacement; full train split multi-seed validation",
            "one hidden tanh MLP, hidden=32, CPU multi-seed focused validation",
            "eventSafe");
    }
    ...
    static (CandidateSpec Spec, double StopBlend, double NormalGain, double ProductBlend)[] GuardedSpecs() => new[]
    {
        (
            new CandidateSpec(
                ...
                32,
                "event-safe MLP with a post-model stop/static sequence latch cap",
        ...
        (
            new CandidateSpec(
                "mlp_h32_event_safe_seq_latch_cap0p35_gain1p08",
                "MLP+SequenceGuard+LagCalibration",
                "temporal+runtime-state",
                32,
                "event-safe MLP with sequence latch/cap and normal-moving gain calibration",
                "cap0.35 stop latch; outside latch, normal-moving output gain 1.08 to counter underprediction lag",
                "postModelGuard",
                StopCapPx: 0.35),
            StopBlend: 0.0,
            NormalGain: 1.08,
        ...
        (
            new CandidateSpec(
                "mlp_h32_event_safe_seq_latch_cap0p25_gain1p08",
                "MLP+SequenceGuard+LagCalibration",
                "temporal+runtime-state",
                32,
                "event-safe MLP with tighter stop cap and normal-moving gain calibration",
                "cap0.25 stop latch; outside latch, normal-moving output gain 1.08",
                "postModelGuard",
                StopCapPx: 0.25),
            StopBlend: 0.0,
            NormalGain: 1.08,
        ...
        (
            new CandidateSpec(
                "mlp_h32_event_safe_seq_latch_cap0p35_productblend0p25",
                "MLP+SequenceGuard+LagCalibration",
                "temporal+runtime-state",
                32,
                "event-safe MLP with sequence latch/cap and normal-moving product-blend lag calibration",
                "cap0.35 stop latch; outside latch, normal-moving output blends 25% toward product baseline",
                "postModelGuard",
                StopCapPx: 0.35),
            StopBlend: 0.0,
            NormalGain: 1.0,
    ...
    static void AddRun(List<CandidateRun> runs, int seed, Dictionary<string, double> productMetrics, double futureLagEpsilon, object eval)
    {
    ...
    static object BuildSeedSummary(List<CandidateRun> runs) => runs
        ...
        .ToDictionary(
            ...
            g => new
            {
                seedCount = g.Count(),
                passCount = g.Count(r => r.PassesIntegrationGate),
                futureLagWithinEpsilonCount = g.Count(r => r.FutureLagWithinEpsilon),
