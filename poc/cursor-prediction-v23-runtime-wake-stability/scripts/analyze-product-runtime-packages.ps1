...
foreach ($package in $packages) {
    ...
    try {
        ...
        $results += [ordered]@{
            ...
            vBlankLeadMicroseconds = Stats ([double[]]@($schedulerRows | ForEach-Object { RowNumber $_ "vBlankLeadMicroseconds" }))
            messageWakeCount = Stats ([double[]]@($schedulerRows | ForEach-Object { RowNumber $_ "messageWakeCount" }))
            fineSleepZeroCount = Stats ([double[]]@($schedulerRows | ForEach-Object { RowNumber $_ "fineSleepZeroCount" }))
            fineSpinCount = Stats ([double[]]@($schedulerRows | ForEach-Object { RowNumber $_ "fineSpinCount" }))
...
$report.Add("")
$report.Add("| variant | controller | updateLayer | tick p95 us | predict p95 us | move p95 us | ULW p95 us | wake-late p95 us | wake-late max us | vblank lead p50 us | message wake avg | fine spin p95 | latest move age p95 us | skipped moves |")
$report.Add("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
foreach ($item in $results) {
    $report.Add("| $($item.variantName) | $($item.controllerCount) | $($item.overlayUpdateLayerCount) | $([Math]::Round($item.controllerTickTotalMicroseconds.p95, 3)) | $([Math]::Round($item.controllerPredictMicroseconds.p95, 3)) | $([Math]::Round($item.controllerMoveOverlayMicroseconds.p95, 3)) | $([Math]::Round($item.overlayUpdateLayerMicroseconds.p95, 3)) | $([Math]::Round($item.wakeLateMicroseconds.p95, 3)) | $([Math]::Round($item.wakeLateMicroseconds.max, 3)) | $([Math]::Round($item.vBlankLeadMicroseconds.p50, 3)) | $([Math]::Round($item.messageWakeCount.avg, 3)) | $([Math]::Round($item.fineSpinCount.p95, 3)) | $([Math]::Round($item.latestMouseMoveAgeMicroseconds.p95, 3)) | $($item.overlayMoveSkippedCount) |")
}
