...
DISPLAY_OFFSET_MS = np.arange(-32.0, 32.1, 4.0, dtype=np.float64)
...
def parse_float_array(source, name):
    match = re.search(rf"{name}\s*=\s*new float\[\]\s*\{{(.*?)\}};", source, re.S)
    if not match:
...
def load_current_smooth_predictor(repo_root):
    source_path = repo_root / "src/CursorMirror.Core/SmoothPredictorModel.g.cs"
    source = source_path.read_text(encoding="utf-8")
    feature_mean = parse_float_array(source, "FeatureMean")
    feature_std = parse_float_array(source, "FeatureStd")
    w0 = parse_float_array(source, "W0").reshape((-1, FEATURE_COUNT))
    b0 = parse_float_array(source, "B0")
    w1 = parse_float_array(source, "W1")
    b1 = parse_float_array(source, "B1")
    hidden = len(b0)
    if w0.shape != (hidden, FEATURE_COUNT) or len(w1) != hidden * 2:
        raise ValueError("Unexpected SmoothPredictor generated model shape")

    def predict(features):
        safe_std = np.where(np.abs(feature_std) < 1.0e-6, 1.0, feature_std)
        normalized = (features - feature_mean) / safe_std
        h = np.tanh(normalized @ w0.T + b0)
        dx = h @ w1[:hidden] + b1[0]
        dy = h @ w1[hidden:] + b1[1]
        return np.column_stack([dx, dy])

    return {
        "modelId": "current_smooth_predictor_generated",
        "hidden": hidden,
        "estimatedMacs": int(w0.size + w1.size),
        "parameters": int(w0.size + b0.size + w1.size + b1.size),
        "featureMean0": float(feature_mean[0]),
        "featureStd0": float(feature_std[0]),
        "predict": predict,
    }
    ...
    def train(self, x, y, directions, epochs, batch_size, lr, seed, loss_kind):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for start in range(0, n, batch_size):
                ...
                if loss_kind.startswith("asym_lead"):
                    lead_weight = float(loss_kind.replace("asym_lead", ""))
                    projection = np.sum(err * db, axis=1)
                    weights = np.where(projection > 0.0, lead_weight, 1.0)
...
def evaluate_predictions(name, pred, y, x, meta, estimated_macs=0, parameters=0, training_trace=None, static_guard=False):
    ...
    static_mask = np.asarray([item["static"] for item in meta], dtype=bool)
    stop_mask = np.asarray([item["slowOrStop"] for item in meta], dtype=bool)
    accepted_mask = np.asarray([item["accepted"] for item in meta], dtype=bool)
    ...
    result = {
        ...
        "overall": {
            ...
            "stopVisualP95": percentile(visual[stop_mask], 95) if stop_mask.any() else 0.0,
            "stopLeadP95": percentile(lead[stop_mask], 95) if stop_mask.any() else 0.0,
            "stopLeadP99": percentile(lead[stop_mask], 99) if stop_mask.any() else 0.0,
            "stopLeadMax": float(lead[stop_mask].max()) if stop_mask.any() else 0.0,
            "acceptedVisualP95": percentile(visual[accepted_mask], 95) if accepted_mask.any() else 0.0,
            "rejectedVisualP95": percentile(visual[rejected_mask], 95) if rejected_mask.any() else 0.0,
        },
        "byHorizon": {},
    }
...
def write_report(path, scores):
    lines = [
        ...
        f"- target correction buckets: {len(scores['config']['targetBuckets'])}",
    ...
    for item in scores["testResults"]:
        ...
        lines.append(
            ...
            f"{overall['leadP99']:.6f} | {overall['stopLeadP99']:.6f} | {overall['stationaryJitterP95']:.6f} | "
...
def main():
    ...
    test_meta = select_meta(meta, test_idx)

    candidates = []
    cv_train_pred = x[train_idx][:, [1, 2]] * 8.0
    cv_test_pred = x[test_idx][:, [1, 2]] * 8.0
    cv12_test_pred = x[test_idx][:, [13, 14]] * 8.0
    candidates.append(evaluate_predictions("constant_velocity_v2", cv_test_pred, test_y, x[test_idx], test_meta, estimated_macs=2, parameters=0))
    candidates.append(evaluate_predictions("constant_velocity_v2_static_guard", cv_test_pred, test_y, x[test_idx], test_meta, estimated_macs=2, parameters=0, static_guard=True))
    candidates.append(evaluate_predictions("constant_velocity_v12", cv12_test_pred, test_y, x[test_idx], test_meta, estimated_macs=2, parameters=0))
    candidates.append(evaluate_predictions("constant_velocity_v12_static_guard", cv12_test_pred, test_y, x[test_idx], test_meta, estimated_macs=2, parameters=0, static_guard=True))

    smooth = load_current_smooth_predictor(repo_root)
    smooth_test_pred = smooth["predict"](x[test_idx])
    candidates.append(
    ...
    candidates.append(
        evaluate_predictions(
            "current_smooth_predictor_static_guard",
            smooth_test_pred,
            test_y,
            x[test_idx],
            test_meta,
    ...
    for hidden, loss_kind in [(32, "mse"), (64, "mse"), (64, "asym_lead4"), (64, "asym_lead8"), (128, "mse")]:
    ...
    scores = {
        "schemaVersion": "cursor-prediction-v25-step-01-runtime-horizon-model-search/1",
        ...
        "config": {
            ...
            "currentSmoothPredictor": {
                "modelId": smooth["modelId"],
                "featureMean0Horizon": smooth["featureMean0"],
                "featureStd0Horizon": smooth["featureStd0"],
            },
        },
        "dataset": {
            "rows": int(len(y)),
        ...
        "limitations": [
            "This run uses a capped sample and reports the test split first.",
            "LeastSquares is approximated by constant-velocity windows in this first script; a direct product-faithful LS replay is deferred.",
            "The MLP loss is row-level MSE or a simple asymmetric lead penalty; full sequence loss is deferred.",
